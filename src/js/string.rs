...
pub fn get_string_length(this:Value, _:Value, _:Vec<Value>) -> ResultValue {
	let this_str: ~str = from_value(this).unwrap();
...
pub fn _create(global: Value) -> Value {
	...
	proto.borrow().set_prop(~"length", prop);
	string.borrow().set_field(~"prototype", proto);
	string
}
/// Initialise the global object with the `String` object
pub fn init(global:Value) {
	global.borrow().set_field(~"String", _create(global));
}